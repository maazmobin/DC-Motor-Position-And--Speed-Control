...
{
  float reading = 11*3.3*(analogRead(A0))/4096; // 1K/(1K+10K)=0.09
  vSum -= vBuffer[0];
  vBuffer.push(reading);
  vSum += vBuffer[buffer_size-1];
  vAverage = vSum/buffer_size;
  if(DEBUG){
  Serial.print(vBuffer[0]);
  Serial.print(" , ");
  Serial.print(vBuffer[buffer_size-1]);
...
void MeasureMotorCurrent(void)
{
  ...
  iAverage = iSum/buffer_size;
  if(DEBUG){
  Serial.print(iBuffer[0]);
  Serial.print(" , ");
  Serial.print(iBuffer[buffer_size-1]);
